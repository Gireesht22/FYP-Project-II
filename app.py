"""
FYP Universal Extractor - Semester 8
...
app.config['SECRET_KEY'] = 'fyp-extractor-sem8'
...
def api_financial_analyze():
    """Financial document analysis — bank statements, invoices"""
    data = request.get_json(silent=True) or {}
    text = data.get('text','').strip()
    doc_type = data.get('doc_type','auto')
    if not text: return jsonify({"error":"No text provided"}),400
    try:
        from extractors.financial_analyzer import analyze_financial_document
        result = analyze_financial_document(text, doc_type)
        result['processed_at'] = datetime.now().isoformat()
        return jsonify(result)
    except Exception as e: return jsonify({"error":str(e)}),500
...
@app.route('/api/financial/export/excel', methods=['POST'])
def api_financial_export_excel():
    """Export financial analysis as Excel file"""
    data = request.get_json(silent=True) or {}
    fin_data = data.get('financial_data', {})
    if not fin_data:
        return jsonify({"error": "No financial data provided"}), 400
    try:
        from extractors.financial_exporter import export_to_excel
        import io
        from flask import send_file
        output = export_to_excel(fin_data)
        output.seek(0)
        return send_file(
            output,
...
@app.route('/api/financial/export/pdf', methods=['POST'])
def api_financial_export_pdf():
    ...
    if not fin_data:
        return jsonify({"error": "No financial data provided"}), 400
    try:
        from extractors.financial_exporter import export_to_pdf
        from flask import send_file
        output = export_to_pdf(fin_data)
        output.seek(0)
        return send_file(
            output,
